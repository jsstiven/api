...
public interface AgendacionRepository extends JpaRepository<AgendacionModel, Integer> {
    ...
    @Query(value = "UPDATE public.agendacion SET tema=:tema, grupo=:grupo, cces=:cedula, fecha=:fecha, hora=:hora WHERE idagendacion = :idAgendacion", nativeQuery = true)
    AgendacionModel editarAgenda(@Param("tema") String tema, @Param("grupo") String grupo,
            @Param("cedula") Integer cedula, @Param("fecha") Date fecha, @Param("hora") Time hora,
