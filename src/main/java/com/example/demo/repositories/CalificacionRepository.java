...
public interface CalificacionRepository extends JpaRepository<CalificacionModel, Integer> {
    ...
    @Query(value = "select json_build_object('Promedio', cast(sum(ca.puntuacion) as float)/cast(count(ca.puntuacion) as float), 'Mes', to_char(es.dia, 'TMMonth'),'Asignatura',es.asignatura) from calificacion ca join asistenciases es ON es.idasistenciases = ca.idasistenciases where ca.id_roles_has_usuarios = :idtutor and es.asignatura = :asignatura group by to_char(es.dia, 'TMMonth'), es.id_roles_has_usuarios,es.asignatura", nativeQuery = true)
    public List<Object> listarCalificacionMensualAsignatura(@Param("idtutor") Integer idTutor, @Param("asignatura") String asignatura) throws JSONException;
